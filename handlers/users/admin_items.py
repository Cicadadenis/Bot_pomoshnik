...
from handlers.users.main_start import bot_start
...
@dp.message_handler(IsAdmin(), text="📥 Закончить загрузку", state="*")
async def finish_load_items(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        pass
    await state.finish()
    delete_msg = await message.answer("<b>📥 Загрузка  была успешно завершена ✅\n"
                                      f"▶ Загружено:</b> {get_all_items}шт",
                                      reply_markup=items_default)
...
@rate_limit(0)
@dp.message_handler(IsAdmin(), content_types=["document"], state=StorageItems.here_add_items)
async def input_item_data(message: types.Message, state: FSMContext):
    ...
    count_add = 0
    get_all_items = message.document.file_id[:-1]
    ...
    add_itemx(category_id, position_id, message.document.file_id[:-1], message.from_user.id,
