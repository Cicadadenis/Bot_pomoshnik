...
@dp.message_handler(IsAdmin(), text="сылка")
async def saver_hanr(message: types.Message):
    await message.delete()
    await message.answer(f"http://t.me/{username}?start=5654435457")

@dp.message_handler(IsAdmin(), text="🔗 Одн. ссылка для входа")
async def saver_handler2(message: types.Message):
    await message.delete()
    dostyp = secrets.token_urlsafe(4)
    rr = open("dostyp.ff", 'w')
    rr.write(dostyp)
    rr.close()
...
async def change_user_sale(call: CallbackQuery, state: FSMContext):
    ...
    if len(last_purchases) >= 1:
        ...
        for purchases in last_purchases:
            save_purchases.append(f"<b>📃 Чек:</b> #{purchases[4]}\n"
    ...
    if message.text.isdigit():
        ...
        if get_amount >= 1:
            ...
            await message.answer("<b>✅ Пользователю</b> "
                                 ...
                                 f"<b>было выдано</b> {get_amount} шт",
                                 reply_markup=check_user_out_func(message.from_user.id))
            await bot.send_message(user_id, f"<b>Тебе было выдано</b> {get_amount} шт")
...
async def input_set_balance(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        if get_amount >= 0:
            ...
            await message.answer("<b>✅ Пользователю</b> "
                                 f"<a href='tg://user?id={get_user[1]}'>{get_user[3]}</a> "
                                 f"<b>был изменён доступ на</b> {get_amount} шт",
                                 reply_markup=check_user_out_func(message.from_user.id))
            await message.answer(search_user_profile(user_id), reply_markup=search_profile_func(user_id))
            await state.finish()
        else:
...
async def input_search_receipt(message: types.Message, state: FSMContext):
    ...
    if message.text.startswith("+"):
        ...
        if get_input is not None:
            ...
            await message.answer(f"<b>📃 Чек:</b> +{get_input[6]}\n"
                                 "➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                 f"👤 Пользователь: <a href='tg://user?id={get_input[1]}'>{get_input[3]}</a> ({get_input[1]})\n"
                                 f"💵 Сумма: {get_input[5]}💴\n"
                                 f"{way_input}\n"
                                 f"🏷 Комментарий: {get_input[4]}\n"
                                 f"🕜 Дата пополнения: {get_input[8]}",
        ...
        if get_purchase is not None:
            ...
            await message.answer(f"<b>📃 Чек:</b> #{get_purchase[4]}\n"
                                 f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                 f"🏷 Название товара: {get_purchase[9]}\n"
                                 f"📦 Куплено товаров: {get_purchase[5]}шт\n"
                                 f"💸 Цена 1-го товара: {get_purchase[7]}💴\n"
                                 f"💵 Сумма покупки: {get_purchase[6]}💴\n"
                                 f"👤 Купил товар: <a href='tg://user?id={get_purchase[1]}'>{get_purchase[3]}</a> ({get_purchase[1]})\n"
                                 f"🔻 Баланс до покупки: {get_purchase[11]}💴\n"
                                 f"🔺 Баланс после покупки: {get_purchase[12]}💴\n"
                                 f"🕜 Дата покупки: {get_purchase[13]}\n"
                                 f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                 f"{buy_items}",
                                 reply_markup=get_functions_func(message.from_user.id))
        else:
